...
<div class="space-y-6">
    <div class="text-center">
        <h1 class="text-xl font-semibold text-slate-800 dark:text-slate-100">Definir nova senha</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Escolha uma nova senha segura.</p>
    </div>
    <form method="POST" action="/reset" class="space-y-4">
        ...
        <div class="space-y-1">
            ...
            <input id="email" name="email" type="email" required autocomplete="email" value="<?php echo htmlspecialchars($email, ENT_QUOTES, 'UTF-8'); ?>" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-400/40 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100" placeholder="voce@exemplo.com">
        </div>
        <div class="space-y-1">
            <label for="password" class="text-sm font-medium text-slate-700 dark:text-slate-200">Nova senha</label>
        ...
        <button type="submit" class="w-full rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-500">Salvar nova senha</button>
    </form>
